set --export POSH_THEME /home/a.marcq/.config/posh/theme.omp.json
...
function fish_prompt
    ...
    if test "$omp_transient" = "1"
      /usr/local/bin/oh-my-posh print transient --config $POSH_THEME --shell fish --status $omp_status_cache --pipestatus="$omp_pipestatus_cache" --execution-time $omp_duration --stack-count $omp_stack_count --shell-version $FISH_VERSION --no-status=$omp_no_exit_code
      return
    end
    ...
    /usr/local/bin/oh-my-posh print primary --config $POSH_THEME --shell fish --status $omp_status_cache --pipestatus="$omp_pipestatus_cache" --execution-time $omp_duration --stack-count $omp_stack_count --shell-version $FISH_VERSION --cleared=$omp_cleared --no-status=$omp_no_exit_code
end
...
function fish_right_prompt
    ...
    set has_omp_tooltip false
    /usr/local/bin/oh-my-posh print right --config $POSH_THEME --shell fish --status $omp_status_cache --execution-time $omp_duration --stack-count $omp_stack_count --shell-version $FISH_VERSION
end
...
function _render_tooltip
  ...
  set omp_tooltip_prompt (/usr/local/bin/oh-my-posh print tooltip --config $POSH_THEME --shell fish --status $omp_status_cache --shell-version $FISH_VERSION --command $omp_tooltip_command)
